...
pub struct QueryAs<'q, 'a, DB: Database, O, A> {
    pub(crate) inner: Query<'q, 'a, DB, A>,
    pub(crate) output: PhantomData<O>,
}

impl<'q, 'a, 'qa, DB, O: Send, A: Send> Execute<'q, 'a, DB> for QueryAs<'q, 'a, DB, O, A>
where
    DB: Database,
...
impl<'q, 'a, 'qa, DB: Database, O> QueryAs<'q, 'a, DB, O, <DB as HasArguments<'a>>::Arguments> {
...
impl<'q, 'a, 'qa, DB, O, A> QueryAs<'q, 'a, DB, O, A>
where
    DB: Database,
    A: 'a + IntoArguments<'a, DB>,
...
pub fn query_as<'q, 'a, 'qa, DB, O>(
    sql: &'q str,
) -> QueryAs<'q, 'a, DB, O, <DB as HasArguments<'a>>::Arguments>
where
    DB: Database,
...
/// using [`FromRow`].
#[inline]
pub fn query_as_with<'q, 'a, 'qa, DB, O, A>(sql: &'q str, arguments: A) -> QueryAs<'q, 'a, DB, O, A>
where
    DB: Database,
...
pub(crate) fn query_statement_as<'q, 'a, 'qa: 'q + 'a, DB, O>(
...
pub(crate) fn query_statement_as_with<'q, 'a, 'qa, DB, O, A>(
    statement: &'qa <DB as HasStatement<'q, 'a>>::Statement,
    arguments: A,
) -> QueryAs<'q, 'a, DB, O, A>
where
